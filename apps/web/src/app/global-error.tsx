        ...
        <div className="max-w-md mx-auto bg-white/90 backdrop-blur rounded-3xl shadow-2xl border border-red-100 p-8 text-center space-y-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center pt-2">
            <button
              type="button"
              onClick={() => reset()}
              className="px-5 py-2.5 rounded-xl bg-red-600 text-white text-sm font-bold shadow-md hover:bg-red-700 transition-colors"
            >
              {isAr ? '🔄 إعادة المحاولة' : '🔄 Try again'}
            </button>
            <a
              href={`/${locale}`}
              className="px-5 py-2.5 rounded-xl border border-red-200 text-sm font-bold text-red-700 hover:bg-red-50 transition-colors"
            >
