...
async function getHalqa(id: string): Promise<any> {
    try {
...
export default async function HalqaDetailPage({ params }: { params: { id: string } }) {
    ...
    const halqa = await getHalqa(params.id);
                            ...
                            <div className="flex flex-col gap-6">
                                <div className="p-8 bg-gradient-to-br from-primary to-primary-light rounded-3xl text-white shadow-btn">
                                    ...
                                    <p className="text-white/80 text-sm mb-6 leading-relaxed">
                                        {locale === 'ar'
                                            ? 'تواصل مباشرة مع المشرف عبر واتساب للافسار عن طريقة التسجيل ومواعيد الحلقة.'
                                            : 'Contact the supervisor directly via WhatsApp for registration and schedule inquiries.'}
                                    </p>
                                    <a href={getWhatsAppUrl(halqa.whatsapp)} target="_blank" rel="noopener noreferrer" className="w-full bg-white text-primary rounded-2xl py-4 font-black flex items-center justify-center gap-3 shadow-lg hover:scale-[1.02] active:scale-95 transition-all text-lg">
