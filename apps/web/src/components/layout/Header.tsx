            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    <Link href={`/${locale}`} className="flex items-center gap-2.5 group">
                        <div className="w-10 h-10 bg-gradient-to-br from-primary to-primary-light rounded-xl flex items-center justify-center text-white text-xl shadow-[0_4px_12px_rgba(27,107,69,0.3)] transition-transform group-hover:scale-105">🕌</div>
                    ...
                    <nav className="hidden md:flex items-center gap-1">
                        {navLinks.map((link) => (
                            <Link key={link.href} href={link.href} className="px-4 py-2 rounded-lg text-text font-bold text-sm transition-all hover:bg-primary/5 hover:text-primary">{link.label}</Link>
                        ))}
                    </nav>
                    ...
                    <div className="flex items-center gap-3">
                        {token && isAdminPath && (
                            <div className="relative">
                                <button onClick={() => setShowNotifications((v) => !v)} className="relative p-2 rounded-btn hover:bg-gray-100 transition-colors" aria-label="Notifications">
                                    <span className="text-xl">🔔</span>
                                    {unreadCount > 0 && (
                                        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-red-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center">{unreadCount > 99 ? '99+' : unreadCount}</span>
                                    )}
                                </button>
                                {showNotifications && (
                                    <div className="absolute end-0 mt-2 w-80 bg-white border border-border rounded-xl shadow-xl z-[1200] p-2">
                                        {items.slice(0, 5).length ? items.slice(0, 5).map((n) => (
                                            <div key={n.id} className="p-2 rounded-lg hover:bg-cream text-xs border-b border-border last:border-b-0">
                                                <p className="font-bold">{n.title}</p>
                                                <p className="text-text-muted">{n.message}</p>
                                                <div className="mt-1 flex gap-2">
                                                    {!n.read && (
                                                        <button className="underline text-primary" onClick={async () => {
                                                            if (token) await adminApi.markNotificationRead(token, n.id).catch(() => undefined);
                                                            markRead(n.id);
                                                        }}>{locale === 'ar' ? 'مقروء' : 'Read'}</button>
                                                    )}
                                                    <button className="underline text-primary" onClick={() => openModal('view', (n.type.includes('imam') ? 'imam' : n.type.includes('halqa') ? 'halqa' : 'maintenance') as any, n)}>{locale === 'ar' ? 'فتح' : 'Open'}</button>
                                                </div>
                                            </div>
                                        )) : <p className="p-2 text-xs text-text-muted">{locale === 'ar' ? 'لا إشعارات' : 'No notifications'}</p>}
                                        <Link href={`/${locale}/admin/notifications`} className="block mt-2 text-center text-xs text-primary underline">{locale === 'ar' ? 'عرض الكل' : 'View all'}</Link>
                                    </div>
                                )}
                            </div>
                        )}
